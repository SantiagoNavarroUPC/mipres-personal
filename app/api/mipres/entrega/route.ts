...
import type { TipoConsultaEntrega } from "@/requests/mipres-sispro/entrega.request"
import {
  consultarEntrega,
  consultarEntregaPorRangoFechas,
} from "@/controllers/mipres-controller/entrega-controller/entrega.controller"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const tipo = searchParams.get("tipo") as TipoConsultaEntrega | "rango"
    ...
    if (tipo === "rango") {
      ...
      if (tieneTokensAcceso) {
        const [resultSubsidiado, resultContributivo] = await Promise.all([
          consultarEntregaPorRangoFechas(credentialsSubsidiado, fechaInicio, fechaFin || undefined, pacienteParams),
          consultarEntregaPorRangoFechas(credentialsContributivo, fechaInicio, fechaFin || undefined, pacienteParams),
        ])

        return buildMergedResponse(
          resultSubsidiado,
          resultContributivo,
          "No se encontraron entregas en el rango especificado"
        )
      }

      // Un solo token
      const credentials = buildCredentials(nit, tokenAcceso, tokenSubsidiado, tokenContributivo)
      const result = await consultarEntregaPorRangoFechas(credentials, fechaInicio, fechaFin || undefined, pacienteParams)
    ...
    if (tieneTokensAcceso) {
      const [resultSubsidiado, resultContributivo] = await Promise.all([
        consultarEntrega(credentialsSubsidiado, tipo, params),
        consultarEntrega(credentialsContributivo, tipo, params),
      ])

      return buildMergedResponse(resultSubsidiado, resultContributivo, "No se encontraron entregas")
    }

    const credentials = buildCredentials(nit, tokenAcceso, tokenSubsidiado, tokenContributivo)
    const result = await consultarEntrega(credentials, tipo, params)
    return buildSingleResponse(result)
  } catch (error) {
    return NextResponse.json(
      {
