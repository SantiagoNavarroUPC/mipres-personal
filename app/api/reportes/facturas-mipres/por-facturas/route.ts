...
export async function POST(request: NextRequest) {
  try {
    ...
    const facturas = Array.isArray(body?.facturas)
      ? body.facturas.map((f: unknown) => String(f ?? "").trim()).filter(Boolean)
      : []

    if (facturas.length === 0) {
      return NextResponse.json(
        { success: false, error: "Debe enviar al menos un número de factura" },
        { status: 400 }
      )
    }

    const endpoint = `${getBackendApiUrl()}/api/reportes/facturas-mipres/por-facturas`
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Error interno al generar el reporte por facturas",
      },
      { status: 500 }
    )
  }
