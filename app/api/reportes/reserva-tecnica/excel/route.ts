  ...
  try {
    ...
    if (!result.success || !result.data) {
      return NextResponse.json(
        { success: false, error: result.error || "No se pudo descargar reporte de reserva tecnica" },
    ...
    return new NextResponse(result.data, {
      ...
      headers: {
        ...
        "Content-Disposition": result.contentDisposition || "attachment; filename=\"reporte-reserva-tecnica.xlsx\"",
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Error interno al descargar reporte de reserva tecnica",
      },
      { status: 500 }
    )
  }
