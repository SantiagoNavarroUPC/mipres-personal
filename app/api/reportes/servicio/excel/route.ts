  ...
  try {
    ...
    if (!result.success || !result.data) {
      return NextResponse.json(
        { success: false, error: result.error || "No se pudo descargar reporte de servicio" },
    ...
    return new NextResponse(result.data, {
      ...
      headers: {
        ...
        "Content-Disposition": result.contentDisposition || "attachment; filename=\"reporte-servicio.xlsx\"",
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Error interno al descargar reporte de servicio",
      },
      { status: 500 }
    )
  }
