  ...
  try {
    ...
    if (!result.success || !result.data) {
      return NextResponse.json(
        { success: false, error: result.error || "No se pudo descargar reporte de producto nutricional" },
    ...
    return new NextResponse(result.data, {
      ...
      headers: {
        ...
        "Content-Disposition": result.contentDisposition || "attachment; filename=\"reporte-producto-nutricional.xlsx\"",
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Error interno al descargar reporte de producto nutricional",
      },
      { status: 500 }
    )
  }
