  ...
  try {
    ...
    if (!result.success || !result.data) {
      return NextResponse.json(
        { success: false, error: result.error || "No se pudo descargar reporte detalles de direccionamiento" },
    ...
    return new NextResponse(result.data, {
      ...
      headers: {
        ...
        "Content-Disposition": result.contentDisposition || "attachment; filename=\"reporte-detalle-mipres.xlsx\"",
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Error interno al descargar reporte detalles de direccionamiento",
      },
      { status: 500 }
    )
  }
