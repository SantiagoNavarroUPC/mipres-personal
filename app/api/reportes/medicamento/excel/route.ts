  ...
  try {
    ...
    if (!result.success || !result.data) {
      return NextResponse.json(
        { success: false, error: result.error || "No se pudo descargar reporte de medicamento" },
    ...
    return new NextResponse(result.data, {
      ...
      headers: {
        ...
        "Content-Disposition": result.contentDisposition || "attachment; filename=\"reporte-medicamento.xlsx\"",
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Error interno al descargar reporte de medicamento",
      },
      { status: 500 }
    )
  }
