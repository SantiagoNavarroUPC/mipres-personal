...
import { AlertTriangle, User, Calendar, Hash, ArrowLeft, Truck } from "lucide-react"
...
export function EntregaLecturaModal({
  ...
  const fields: { label: string; value: string | number }[] = [
    ...
    { label: "ID Entrega", value: current?.IDEntrega ?? "N/A" },
    { label: "Número de Entrega", value: current?.NoEntrega ?? "N/A" },
    { label: "Código Servicio/Tecnología Entregado", value: current?.CodSerTecEntregado ?? "N/A" },
    { label: "Cantidad Total Entregada", value: current?.CantTotEntregada ?? "N/A" },
    {
      label: "Entrega Total",
      value: current?.EntTotal === undefined || current?.EntTotal === null
        ? "N/A"
        : Number(current.EntTotal) === 1 ? "Sí" : "No",
    },
    { label: "No. Lote", value: current?.NoLote || "No tiene Lote" },
    {
      label: "Recibe",
      value: current?.TipoIDRecibe && current?.NoIDRecibe
  ...
  if (causaNoEntrega > 0) {
    fields.push({
      label: "Causa No Entrega",
      value: (CAUSAS_NO_ENTREGAS as Record<number, string>)[causaNoEntrega] ?? causaNoEntrega,
    })
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-amber-100">
        ...
        <div className="min-w-0">
          <h2 className="text-base font-semibold leading-none">Detalle de Entrega</h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            Prescripción <span className="font-mono font-semibold text-primary">{noPrescripcion}</span>
            {sortedEntregas.length > 1 && (
              <> · Registro <span className="font-semibold">{page}</span> de <span className="font-semibold">{sortedEntregas.length}</span></>
            )}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-lg border border-[#f5e6b5] bg-amber-100 p-4 flex items-start gap-3 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <div>
            ...
            <div className="font-semibold text-base">{formatDateTime(current?.FecEntrega)}</div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-amber-100 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className={`mt-1 h-10 px-3 py-2 rounded-md border text-sm flex items-center ${getEstadoEntregaClass(current?.EstEntrega)}`}>
