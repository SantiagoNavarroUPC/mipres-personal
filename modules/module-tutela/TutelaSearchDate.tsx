...
interface TutelaSearchDateProps {
  fechaInicio: string
  fechaFin: string
  hoy: string
  loading: boolean
...
export function TutelaSearchDate({
  fechaInicio,
  fechaFin,
  hoy,
  loading,
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Ingrese solo la fecha de inicio para buscar un dia especifico. Si tambien ingresa la fecha fin, se consultara todo el rango en paralelo.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div>
            <Label htmlFor="fechaInicioTut">Fecha inicio</Label>
            <Input
              id="fechaInicioTut"
              type="date"
              value={fechaInicio}
          ...
          <div>
            <Label htmlFor="fechaFinTut">Fecha fin (opcional)</Label>
            <Input
              id="fechaFinTut"
              type="date"
              value={fechaFin}
        ...
        {loadingRango && (
          <div className="space-y-2 rounded-lg border border-border bg-muted/30 p-4">
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground font-medium">Progreso de consulta por hilos</span>
        ...
        {!loadingRango && rangoProgress === 100 && rangoInfo && (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3">
            <p className="text-sm text-emerald-700 font-medium">{rangoInfo}</p>
          </div>
        )}
      </CardContent>
    </Card>
